...
!   JanimationSub()  - Callback routine for the main dialog box
!   JanimationApply()- Callback routine for the APPLY button
!
...
!DEC$ ENDIF
    ...
    external JanimationSub
    external JanimationApply
    ...
    lret = DlgInit(IDD_JANIMATION_DIALOG, gdlg)
    if (lret == .FALSE.) goto 99999
    lret = DlgSetSub(gdlg, IDD_JANIMATION_DIALOG, JanimationSub)
    lret = DlgSetSub(gdlg, IDM_APPLY, JanimationApply)
...
99999 &
    ...
    ret = MessageBox(ghwndMain, "Error initializing application Janimation"C, &
                     "Error"C, MB_OK)
    WinMain = 0
...
!****************************************************************************
!
!  FUNCTION: JanimationSub ( dlg, id, callbacktype )
!
...
SUBROUTINE JanimationSub( dlg, id, callbacktype )
!DEC$ ATTRIBUTES DEFAULT :: JanimationSub
  ...
  END SUBROUTINE JanimationSub

!****************************************************************************
!
!  FUNCTION: JanimationApply ( dlg, id, callbacktype )
!
...
SUBROUTINE JanimationApply( dlg, id, callbacktype )
!DEC$ ATTRIBUTES DEFAULT :: JanimationApply
  ...
  character(256) :: value
  integer(4) :: ret
  real :: rec, imc, xmin, xmax, ymin, ymax, zmax, zmaxmin, zmaxmax, zmaxstep
  integer :: colors, dimm
...
!	   call fglColor3f((r/real(colors)),(g/real(colors)),(bl/real(colors)))
  ...
  END SUBROUTINE JanimationApply
